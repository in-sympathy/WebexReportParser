...
function main(workbook: ExcelScript.Workbook): void {
  ...
  // Filter: Only keep rows where the "Display Name" column contains one of the keywords.
  const displayNameColumn: string = "Display Name";
  const displayNameIdx: number = headers.indexOf(displayNameColumn);
  // If the "Display Name" column exists, filter based on it.
  if (displayNameIdx !== -1) {
    const keywords: string[] = ["comfy", "comfi", "комфі"];
    const filteredRows: CellValue[][] = consolidatedRows.filter((row: CellValue[]) => {
      const displayName: string = (row[displayNameIdx] || "").toString();
      return keywords.some((keyword: string) =>
        displayName.toLowerCase().includes(keyword)
      );
    });

    // If filtered rows exist, overwrite the data and reapply the Percentage formulas.
